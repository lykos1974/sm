...
Independently re-audit the MEXC order-discovery evidence aliases. For filled records, every supplied alias of order ID, native symbol, side, requested and filled quantity, average fill price, status and update timestamp must normalize consistently; conflicting or malformed aliases must fail as RESPONSE_SCHEMA. Optional fill timestamp aliases must agree, but the historical order response alone does not establish an execution timestamp, so output fill_time remains null. Verify sanitized diagnostics, GET allowlist, canonical signing, duplicate-key rejection and redirect denial. Adapter, shadow check, fill reconciliation, trader, validation and alerts remain OFF.
