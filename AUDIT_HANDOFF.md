...
Independently re-audit MEXC order discovery against historical responses: every returned row must have integer state 3 and complete, consistent identity, symbol, side, requested and filled quantities, average fill price and update timestamp. Any other state, missing evidence or conflicting alias rejects the entire page as RESPONSE_SCHEMA without exposing response data. The historical order response does not establish an execution timestamp, so output fill_time remains null. Verify sanitized diagnostics, GET allowlist, canonical signing, duplicate-key rejection and redirect denial. Adapter, shadow check, fill reconciliation, trader, validation and alerts remain OFF.
