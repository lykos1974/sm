...
Independently audit the separate MEXC read-only reconciliation observer before any Windows packaging. Current live_trades has exchange_order_id but lacks dedicated external_oid, native_symbol, requested_quantity, submitted_at_ms and bot_generated provenance; the audited order-status adapter also omits external_oid from normalized evidence. These are complete-binding gaps: current rows must classify INSUFFICIENT_BINDING without an exchange request, and no live full-fill match can be claimed. Verify URI read-only, query_only, authorizer, WAL refusal, bounded sequential GET use, chronological exact-Decimal evidence, deterministic digests and exclusive sanitized reports against offline fixtures. No DB migration, operational reconciliation, trader, validation or alerts; all remain OFF. Protected baseline remains untouched.
