...
Independently re-audit the MEXC shadow report on Windows: GetFileAttributesW must return DWORD with captured last error; only file/path-not-found permits a new target, while access denial, existing files and reparse points fail closed. Confirm that legitimate new relative and absolute paths with spaces reach same-directory creation and exclusive publication; a competing target remains untouched. Do not claim directory fsync. Recheck the read-only shadow request with synthetic evidence only, then pin repaired bytes in a separate reversible package stage. Adapter, fill reconciliation, trader, validation and alerts remain OFF.
