...
Independently re-audit MEXC filled-history order discovery: every returned row must validate integer state 3, integer side 1–4, matching symbol, complete quantities, average price and update timestamp before opening sides 1/3 are output. Closing sides 2/4 must be validated and excluded from entry candidates; any malformed row rejects the entire response. Check bounded diagnostic failure categories against a sanitized real-world schema description without exposing exchange data. The historical response does not prove execution time; output fill_time remains null. Verify GET allowlist, signing, redirect denial and duplicate-key rejection. Adapter, shadow check, fill reconciliation, trader, validation and alerts remain OFF.
