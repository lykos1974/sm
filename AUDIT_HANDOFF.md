...
Independently re-audit the standalone MEXC order-discovery CLI's sanitized diagnostic classifications. Verify offline that HTTP_STATUS, AUTH_REJECTED, API_REJECTED, RESPONSE_SCHEMA, NETWORK_ERROR and INTERNAL_ERROR reflect the actual failure boundary, including valid HTTP responses with non-2xx status, and that optional metadata contains only the fixed endpoint label, bounded HTTP status and schema stage. Confirm no response body, credential, signature, header or account data can escape. Preserve the GET allowlist, canonical signing, strict evidence checks and redirect denial. The history response supplies updateTime but no proven fill timestamp; fill_time stays null. Adapter, shadow check, fill reconciliation, trader, validation and alerts remain OFF.
