...
Independently re-audit MEXC order-discovery alias validation across filled and skipped non-filled states 1, 2, 4 and 5. Every supplied identity, side, quantity, average price, status or timestamp alias must agree even when a row is skipped; zero deal quantity and price remain valid for non-filled rows. Conflicts fail as RESPONSE_SCHEMA without exposing response data. The historical order response does not establish an execution timestamp, so output fill_time remains null. Verify sanitized diagnostics, GET allowlist, canonical signing, duplicate-key rejection and redirect denial. Adapter, shadow check, fill reconciliation, trader, validation and alerts remain OFF.
