...
def _classify_regime(pattern_name: str, fav: list[float], adv: list[float], future_obs: int, sideways_cols: int = 3) -> str:
    ...
    if max_adv >= 3 and early_adv_max >= 2 and early_fav_max <= 1:
        return REGIME_FAST_MEAN_REVERSION

    if max_fav >= 3 and early_fav_max >= 2 and early_adv_max <= 1:
        return REGIME_TREND_CONTINUATION

    if max_adv >= 3 and max_fav <= 2:
        return REGIME_FAILED_REVERSAL

    early_sideways = all((f < 2 and a < 2) for f, a in zip(early_fav, early_adv))
    late_breakout = (max(later_fav) if later_fav else 0) >= 3
    if early_sideways and late_breakout:
        return REGIME_TREND_CONTINUATION
    ...
    return REGIME_VOLATILE_CHOP if (max_fav > 0 and max_adv > 0) else REGIME_SIDEWAYS_COMPRESSION


def _compute_followthrough(row: dict[str, str], future_columns: int) -> FollowthroughMetrics:
    favorable = [
        _to_float(row.get(f"fav_col_{i}"), 0.0)
        for i in range(1, future_columns + 1)
        if f"fav_col_{i}" in row
    ]
    adverse = [
        _to_float(row.get(f"adv_col_{i}"), 0.0)
        for i in range(1, future_columns + 1)
        if f"adv_col_{i}" in row
    ]
    ...
    early_range = [(f + a) for f, a in zip(favorable[:early_window], adverse[:early_window])]
    ...
    compression = _safe_ratio(later_mean, early_mean) if early_mean > 0 else 0.0
...
def main() -> None:
    ...
    for row in rows:
        ...
        fav_path = [_to_float(row.get(f"fav_col_{i}"), 0.0) for i in range(1, args.future_columns + 1)]
        adv_path = [_to_float(row.get(f"adv_col_{i}"), 0.0) for i in range(1, args.future_columns + 1)]
        regime = _classify_regime(row.get("pattern_name", ""), fav_path, adv_path, future_obs)

        regime_rows.append(
            {
                "symbol": row.get("symbol", ""),
                "timestamp": row.get("reversal_timestamp", row.get("timestamp", "")),
