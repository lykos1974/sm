...
class MexcFuturesClient:
    ...
    def get_position_mode(self) -> dict[str, Any]:
        return self._request_json("GET", "/api/v1/private/position/position_mode", signed=True)
...
def init_live_tables(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS live_trader_state (
...
def get_last_processed_close_time(conn: sqlite3.Connection, symbol: str) -> int | None:
    row = conn.execute(
        "SELECT last_processed_close_time FROM live_trader_state WHERE symbol = ?",
        (symbol,),
    ).fetchone()
...
def set_last_processed_close_time(conn: sqlite3.Connection, symbol: str, close_time: int) -> None:
    conn.execute(
        """
        INSERT INTO live_trader_state(symbol, last_processed_close_time, updated_at)
        VALUES(?,?,?)
...
def build_reduce_only_close_order(
    *,
    ...
    if "vol" not in entry_order:
        return None, "missing entry order volume for reduce-only close"
    ...
    price = quantize_down(exit_price, spec.price_unit, spec.price_scale)
    vol = _dec(entry_order["vol"])
    if not aligned(price, spec.price_unit, spec.price_scale):
    ...
    return (
        {
            ...
            "type": 1,
            "openType": 1,
            "externalOid": f"pnf-exit-{trade_id}"[:32],
            "positionMode": 2,
            "reduceOnly": True,
        },
        None,
    ...
    for row in rows:
        ...
        for close_time, high, low in candles:
            ...
            try:
                ...
                if not mode_response.get("success") or mode_response.get("data") != 2:
                    raise RuntimeError(f"reduceOnly is only accepted for one-way mode; position mode response={mode_response}")
                spec = client.get_contract_detail(mexc_contract_symbol(symbol))
                ...
                conn.execute(
                    "UPDATE live_trades SET status = 'EXIT_PENDING', notes = ? WHERE id = ?",
            ...
            if not raw_close_response.get("success") or raw_close_response.get("code") not in (0, None):
                conn.execute(
                    "UPDATE live_trades SET status = 'EXIT_PENDING', notes = ? WHERE id = ?",
                    (f"reduce-only exit rejected: {json.dumps(raw_close_response, sort_keys=True)}", int(trade_id)),
                )
                conn.commit()
                console("ORDER_FAILED", f"{symbol} trade_id={trade_id} reduce-only close", raw_close_response)
                break
...
def validate_guards(
    ...
    *,
    ...
    if live_enabled:
        ...
        if not mode_response.get("success") or mode_response.get("data") != 2:
            return None, None, f"one-way position mode required for reduceOnly exits: {mode_response}"
    ...
    try:
        ...
        for symbol in configured_symbols:
            ...
            try:
                ...
                record_trade(conn, signal, notional_usdt=notional_usdt, exchange_order_id=None, status="ORDER_FAILED", raw_order_response={"order_request": order, "order_response": raw_response}, notes="API/order error; fail closed")
            ...
            if not raw_response.get("success") or raw_response.get("code") not in (0, None):
                ...
                record_trade(conn, signal, notional_usdt=notional_usdt, exchange_order_id=None, status="ORDER_FAILED", raw_order_response={"order_request": order, "order_response": raw_response}, notes="MEXC returned unsuccessful order response")
                console("ORDER_FAILED", f"{signal.symbol} {signal.pattern}", raw_response)
                continue
            ...
            record_trade(conn, signal, notional_usdt=notional_usdt, exchange_order_id=order_id, status="ORDER_SENT", raw_order_response={"order_request": order, "order_response": raw_response}, notes="live MEXC futures aggressive limit order submitted")
